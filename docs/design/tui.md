...
┌─────────────────────────────────────┐
│                                     │
│   (Job pty / compatibility session view)    │
│                                     │
├─────────────────────────────────────┤
│  Job: Ctrl+Z detach                │
│  Bridge: Enter send • Ctrl+C cancel │
└─────────────────────────────────────┘
```
...
| Job :fg 退出键 | Ctrl+Z |
| 兼容会话 :fg 控制 | Enter 发送 follow-up；Ctrl+C cancel 当前轮；Ctrl+Z 本地 detach |
...
- 选中后智能路由：
  ...
  - 已结束的 Job → 自动 `:out`
  - AGENT → 自动请求 `:fg A<n>`；这是兼容桥接的会话视图，不是核心模式
...
- 错误 → 红色文字
- `:ask` / `:spawn` / `:send A1 ...` → 下半只显示轻量 ack / status；真实兼容会话内容进入 session view / `:fg A1`
...
| 补全 | Tab |
| 模式切换 | Shift+Tab 在任意非-FG 焦点循环 JOB→CRON→AGENT(compat) |
...
|------|------|
| 会话编号 | A1, A2, ...（兼容桥接标识） |
| 会话与 Job 关系 | **兼容 surface**（依赖 weft，不是产品核心原语） |
| 会话 :fg | 打开统一前台会话视图；不再把 streamed/final 内容灌进普通结果卡片 |
| 交互 | 在兼容会话区显示 follow-up，用户 input line 回复 |
