...
cue-shell 最初想做的是一个「人和 agent 都能顺手用的 shell」：一边保留 bash/zsh 那种把进程跑起来、串起来、看着输出的直觉，一边把 AI agent 当成一等公民塞进去——`:ask` / `:spawn` / `:agents` / `:confirm` / `:probe`、planner/executor 模型、ACP backend、agent transcript 全部在 shell 这一层。

跑了一段时间之后，问题变成：当 agent 行为越来越复杂，cue-shell 同时承担「进程运行时」和「agent 协作策略」两份责任，边界开始糊掉。AGENT mode 的策略问题（哪个 agent、哪个 model、什么时候升级、是否需要确认）渗到了 process 层；而 process 层本来该做的事——可靠地 spawn / pipe / chain / 持久化 / 订阅事件——反而被边角化。

随着 loom / weft / warp 三个仓库的角色逐步清晰，整体闭环开始落到 **prompt/schedule → loom durable runtime → weft agent runtime → warp project ops → cue-shell process substrate**。这条链上最底下一层只应该回答一个问题：「我要起一个进程、串一组进程、把它们跑得久一点、可被观察、可被控制」。所以 cue-shell 在这一版主动收缩：把 agent 一等原语、planner/executor、ACP backend lifecycle、agent transcript、agent wake/escalation 全部交出去，让自己回到 bash-like durable process substrate 的位置。
...
- 一个上层 agent runtime（weft）只通过 JSON IPC 与 cue-shell 对话就能完成所有「起进程、读输出、写 stdin、订阅事件、组 chain」的事，不需要 cue-shell 为它加任何 agent 专用接口。
...
- **weft**：agent runtime / control plane + superagent CLI。所有 agent 一等原语（AGENT mode、planner/executor、ACP backend lifecycle、agent transcript、agent wake/escalation、`:ask` / `:spawn` / `:agents` / `:confirm` / `:probe`）从 cue-shell 迁出到 weft。weft 通过 cue-shell 的 JSON IPC 起 job、订阅事件，不把 agent 概念下沉。
...
- **不承担 agent-policy 命令**：`:ask` / `:spawn` / `:agents` / `:confirm` / `:escalate` / `:probe` 等不再属于 cue-shell。
...
- 2026-04-26：初稿。从「human-agent collaboration shell」收缩为 bash-like durable process substrate；agent 一等原语、planner/executor、ACP backend lifecycle、agent transcript、agent wake/escalation、`:ask` / `:spawn` / `:agents` / `:confirm` / `:probe` 等迁出至 weft；多步 workflow 编排归 loom；warp 退化为被 cue-shell 跑起来的普通可执行。
